...
my ($input,$windows,$min_identity,$len_specific,$type_masked,$help);
GetOptions(
	"input:s"=>\$input,
	"window:i"=>\$windows,
...
for(my $i=0;$i<@id;$i++){
	...
	my $line_tmp='';
	my @pos;$pos[0]=$pos[1]=$pos[2]=0;##$pos[0] the start pos; $pos[1] the length; $pos[2] the flag 0:the prior is end by un'n', 1: is n.
	...
	for(my $k=0;$k<length($line);$k++){
		...
		if($base=~/n|N/){
			...
			$pos[0]=$pos[1]=0;$line_tmp='';$pos[2]=1;
			next;
		}
...
sub check_masked{
	...
	for my $k(keys %{$hash1}){
		...
		my $l2=$hash2->{$k};
		my $l_tm='';
		if(length($l1)!=length($l2)){die "length is no equal\n";}
		for(my $i=0;$i<length($l2);$i++){
			my $base=substr($l2,$i,1);
			if($base eq 'n')
			}
...
sub usage{
	...
	--type_masked(t): the specific sequences masked by 'n' and low identity sequences masked by lowercase. Default both of two type masked by 'n'.
