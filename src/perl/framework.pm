...
sub getTreeError {
    ...
    my $fncmd = setenv::PERL_COMMAND . " " . setenv::COMPARETREE_EXECUTABLE . " -tree $tree2File -versus $tree1File ";
    my $fpcmd = setenv::PERL_COMMAND . " " . setenv::COMPARETREE_EXECUTABLE . " -tree $tree1File -versus $tree2File ";
...
sub getAlignmentError {
    ...
    my $distancedircmd = setenv::JAVA_COMMAND . " gsp.score.DataMatrix -v $workdir/rose.aln.true.fasta -f $estimatedAlignmentFile -sp > $workdir/sp.error";
...
# calculate alignment sp error assuming all inputs are FASTA format
# arguments are:
# 1. working directory pathname
# 2. true alignment filename with path
# 3. estimated alignment filename with path
sub getAlignmentErrorNoConversion {
    my $workdir = shift;
    my $trueAlignmentFile = shift;
    my $estimatedAlignmentFile = shift;
    my $scriptdir = getcwd;
    ...
    chdir setenv::GSP_DIR;
    my $distancedircmd = setenv::JAVA_COMMAND . " gsp.score.DataMatrix -v $trueAlignmentFile -f $estimatedAlignmentFile -sp > $workdir/sp.error";
    framework::systemSafe($distancedircmd);
    chdir $scriptdir;
...
# uses gsp.score.BigDataMatrix infinite precision SP score calculator
# calculate alignment sp error assuming all inputs are FASTA format
# arguments are:
# 1. working directory pathname
# 2. true alignment filename with path
# 3. estimated alignment filename with path
sub getBigAlignmentErrorNoConversion {
    my $workdir = shift;
    my $trueAlignmentFile = shift;
    my $estimatedAlignmentFile = shift;
    my $scriptdir = getcwd;
    ...
    chdir setenv::GSP_DIR;
    my $distancedircmd = setenv::JAVA_COMMAND . " gsp.score.BigDataMatrix -v $trueAlignmentFile -f $estimatedAlignmentFile -sp > $workdir/sp.error";
    framework::systemSafe($distancedircmd);
    chdir $scriptdir;
...
# run AMAC to get AMAe, AMA, and associated alignment errors/scores
# second argument is true alignment
# third argument is estimated alignment
# WARNING - this takes awhile to calculate! ~2 minutes for 1 pair of 100
# taxon alignments, longer for more taxa!
#
# returns reference to array with 2 references
# first is reference to stats name array, then second is reference to 
# stats values array
#
# returns reference to array result
# pass by reference
# it needs to be initialized to empty array before passing in
# order is always
# (amae, ama, f1, f2, i1, i2, d1, d2, id1, id2, agr1, agr2)
# see AMAC code and readme for explanation of these quantities
#
# boolean flag controls whether to use AMA calculator or YAAM calculator
# might add more calculators later?
sub getAmacStatistics {
    my $workdir = shift;
    my $trueAlignmentFile = shift;
    my $estimatedAlignmentFile = shift;
    my $useAMACalculatorFlag = shift;
    my $scriptdir = getcwd;
    ...
    ($fh,$amacstats)=tempfile(DIR => $workdir); close $fh;

    my $amacCmd = "";
    if ($useAMACalculatorFlag) {
	$amacCmd = setenv::AMAC_DIR . "/amac -r $trueAlignmentFile -p $estimatedAlignmentFile > $amacstats";
    }
    else {
	$amacCmd = setenv::YAAM_DIR . "/yaam -r $trueAlignmentFile -p $estimatedAlignmentFile > $amacstats";
    }
    framework::systemSafe($amacCmd);

    # read and chop stats file
    open (AMACSTATS, "$amacstats");
    # first line has stats numbers split by tabs
    my $statsnameline = <AMACSTATS>;
    my $statsline = <AMACSTATS>;
    chomp ($statsnameline);
    chomp ($statsline);
    close (AMACSTATS);

    # testing
    #print "testing: |$statsline|\n";

    my @statsnamelineArray = split(/\s/, $statsnameline);
    my @statslineArray = split(/\s/, $statsline);

    # skip counts
    # 14 counts
    for (my $i = 0; $i < 14; $i++) {
	shift(@statsnamelineArray);
	shift(@statslineArray);
    }
 
# testing   
    if (-e "$amacstats") { framework::systemSafe("rm $amacstats"); }

    my @results = (\@statsnamelineArray, \@statslineArray);

    # now just return reference to result
    return (\@results);
}
...
# do midpoint rooting with paup
# also do internal node name stripping if need be
# WARNING - REMEMBER TO PRESERVE BRANCH LENGTHS on input tree!
#
# all filenames have to have full paths
#
# WARNING - paup branch lengths usually output with scientific notation e?? !
sub midpointRoot {
    my $inputTree = shift;
    my $outputTree = shift;
    my $stripInternalNodeNamesFlag = shift;
    my $workdir = shift;
    my $usePAUPFlag = shift;

    my $fh;
    my $scriptdir = getcwd;
    my $temptree;

    ($fh,$temptree)=tempfile(DIR => $workdir); close $fh;


    if ($stripInternalNodeNamesFlag) {
	# strip internal node names
	chdir setenv::METHODCODE_DIR;
	my $stripcmd = "./strip_internal_node_names_from_newick_tree.pl -i $inputTree -o $temptree.rootme &> /dev/null";
	framework::systemSafe($stripcmd);
	chdir $scriptdir;
    }
    else {
	copy($inputTree, "$temptree.rootme") or warn "$inputTree doesn't exist!";
    }

    # argh - create_paup doesn't allow for full paths since paup scripts
    # don't seem to like it
    my $temptreefilename = framework::getDirectoryLastDirectory($temptree);

    if ($usePAUPFlag) {
	# now do midpoint rooting
	# just go ahead and do midpoint rooting
	chdir setenv::METHODCODE_DIR;
	my $rootpaupcmd = "./create_paup_script_to_root_trees.pl -d $workdir/ -i $temptreefilename.rootme -o $temptreefilename -s $temptreefilename.rootme.paup -b 1 &> /dev/null";
	framework::systemSafe($rootpaupcmd);
	chdir $scriptdir;
	
	# now execute paup midrooting
	# stupid paup can't handle absolute paths in filenames in scripts
	chdir $workdir;
	my $midpointroottreecmd = setenv::PAUP_EXECUTABLE . " -u -n $temptree.rootme.paup ";
	framework::systemSafe($midpointroottreecmd);
	chdir $scriptdir;

	copy($temptree, $outputTree) or warn "$temptree doesn't exist!";
    }
    else {
	# use custom midpoint rooting program
	framework::systemSafe(setenv::RANDOM_REFINE_EXECUTABLE . " -t $temptree.rootme > $temptree");
	  
	  chdir (setenv::GSP_DIR);
	  framework::systemSafe(setenv::JAVA_COMMAND . " gsp.util.MidpointRoot -i $temptree -o $outputTree");
	  chdir ($scriptdir);
	  
    }

    if (-e "$temptree") { framework::systemSafe("rm $temptree"); }
    if (-e "$temptree.rootme") { framework::systemSafe("rm $temptree.rootme"); }
    if (-e "$temptree.rootme.paup") { framework::systemSafe("rm $temptree.rootme.paup"); }
