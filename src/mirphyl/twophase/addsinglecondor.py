...
def copy_files_for_replica(ds_path, out_ds_path, rep, names, alg, ds_name):
    ...
    if os.path.exists(reference_seq_name): 
...
def make_ML_condor_file(outpath, ml_tool,ds_name,ml):
...
if __name__ == '__main__':
    ...
    for ds in datasets:
        ds_paths, replicas, names, outgroup = get_datasets(ds, twophase_conf.get("datasets", ds))
                ...
                for ml in mltools:
                    ...
                    for alg in algtools:
                        ...
                        condor_st = condor_temp_def.format(exe=tool_path,
                                                             ...
                                                             name="distance",
                                                             options="")
                        out = open(os.path.join(out_path, "condor.distance"), "w")
                        out.write(condor_st)
                        out.close()
...
#                        
#                        make_conversion_condor_file(out_path,alg)
#                        
#                        if ml != DUMMY:                                                                                             
#                            make_ML_condor_file(out_path, ml_tool,ds_name,ml)    
#                            if is_data_available(CODE_REFTREE):                    
