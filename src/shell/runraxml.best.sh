...
if [ $C -gt 1 ]; then
 raxmlHPC-PTHREADS-SSE3-7.3.5-64bit $out -m $model -T $C -n best -s $in.phylip -N $N -p $RANDOM &>$H/$id/logs/best
else
 raxmlHPC-SSE3-7.3.5-64bit $out -m $model -n best -s $in.phylip -N $N -p $RANDOM &>$H/$id/logs/best
fi
